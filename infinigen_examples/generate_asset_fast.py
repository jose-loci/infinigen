...
from infinigen.assets.objects.trees.treeconfigs import random_tree
...
def build_scene_asset(args, factory_name, idx, output_folder):
    fac = None
    ...
    with FixedSeed(idx):
        fac = fac(idx, out_dir=output_folder)
        try:
...
def build_and_save_asset(payload: dict):
    ...
    if "Factory" in factory_name:
        asset = build_scene_asset(args, factory_name, idx, output_folder)
    else:
    ...
    print(f"Scene {factory_name} {idx} took {time() - tic:.2f}s")
